{ self, config, inputs, lib, ... }: {
  ...
  flake.modules.homeManager.desktop-hyprland = { pkgs, config, osConfig, ... }: let
    startupScript = pkgs.pkgs.writeShellScriptBin "start" (
      ...
      '' + (lib.optionalString (osConfig.configuration.machine.host == "laptop") ''
  ...
  in {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        source = "${config.home.homeDirectory}/.config/hypr/catppuccin_macchiato.conf";

        general = {
          monitor = [
          ] ++ (lib.optionals (osConfig.configuration.machine.host == "laptop")) [
            "eDP-1, 2880x1800@90, 0x0, 1.8"
          ] ++ (lib.optionals (osConfig.configuration.machine.host == "desktop")) [
